...
Follow the eight numbered steps in order and you will finish with a working page that stitches together two independent micro-frontends at run-time, 
using nothing except HTML 5, plain CSS and vanilla JS.

---

## TL;DR (one-paragraph executive summary)

1. JavaScript orchestration works
   1. by shipping each micro-frontend (MFE)
   2. as a self-contained JavaScript bundle 
   3. that **does not immediately render anything**;

 2. on load it 
    1. **attaches one or more global lifecycle functions**
       1. (e.g. `window.renderHeader`, `window.unmountHeader`)
    2.  a thin “container” page then decides 
        1.  **what** to load 
            1.  usually via simple `<script>` tags, 
            2.  optionally loaded lazily
        2.  **where** to mount it 
            1.  by calling 
                1.  the relevant render function 
            2.  passing the ID
                1.  of the host DOM node
2.  because the contract 
        1.  between the container 
            1.  and 
        2.  each MFE 
    1.  is just a tiny function signature, 
3.  every team can 
    1.  build, test and deploy 
    2.  its slice independently, 
4.  while 
    1.  the container retains control 
        1.  over 
    2.  cross-cutting concerns 
        1.  such as routing, auth and shared CSS scope
5.  this runtime-integration pattern 
    1.  is considered 
6.  the most flexible of the three classic client-side techniques
7.   ([martinfowler.com][1], [newsletter.systemdesign.one][2], [xenonstack.com][3])
