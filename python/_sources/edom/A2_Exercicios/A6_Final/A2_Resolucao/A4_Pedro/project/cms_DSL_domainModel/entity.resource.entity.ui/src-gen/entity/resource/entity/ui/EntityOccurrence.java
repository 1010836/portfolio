...
public class EntityOccurrence {
	...
	public final static String OCCURRENCE_ANNOTATION_ID = "entity.resource.entity.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "entity.resource.entity.ui.occurences.declaration";
	...
	private entity.resource.entity.ui.EntityTokenScanner tokenScanner;
	...
	private entity.resource.entity.IEntityTextResource textResource;
	 ...
	 */
	public EntityOccurrence(entity.resource.entity.IEntityTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, entity.resource.entity.ui.EntityTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new entity.resource.entity.mopp.EntityMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(entity.resource.entity.ui.EntityBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, entity.resource.entity.ui.EntityPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, entity.resource.entity.ui.EntityPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, entity.resource.entity.ui.EntityPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == entity.resource.entity.ui.EntityPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(entity.resource.entity.ui.EntityOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(entity.resource.entity.ui.EntityOccurrence.DECLARATION_ANNOTATION_ID);
	}
