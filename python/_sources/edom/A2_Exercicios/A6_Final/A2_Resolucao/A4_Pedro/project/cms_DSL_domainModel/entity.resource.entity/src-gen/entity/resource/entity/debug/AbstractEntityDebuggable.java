...
public abstract class AbstractEntityDebuggable {
	
	/**
	 * The list of breakpoints, where each breakpoint is represented by its location
	 * (a string) and the line number
	 */
	private java.util.List<entity.resource.entity.util.EntityPair<String, Integer>> lineBreakpoints = new java.util.ArrayList<entity.resource.entity.util.EntityPair<String, Integer>>();
	...
	public void startEventSocket(int eventPort) {
		try {
			...
			new entity.resource.entity.util.EntityRuntimeUtil().logError("Can't create socket connection while launching.", e);
		}
	}
	...
	public void stopEventSocket() {
		try {
			...
			new entity.resource.entity.util.EntityRuntimeUtil().logError("Exception while closing socket.", e);
		}
	}
	
	public void sendEvent(entity.resource.entity.debug.EEntityDebugMessageTypes command, boolean sendOnlyInDebugMode, String... arguments) {
		if (isDebugMode() || !sendOnlyInDebugMode) {
			entity.resource.entity.debug.EntityDebugMessage message = new entity.resource.entity.debug.EntityDebugMessage(command, arguments);
	...
	public void evaluateLineBreakpoint(org.eclipse.emf.common.util.URI uri, int currentLine) {
		if (isDebugMode()) {
			...
			for (int i = 0; i < lineBreakpoints.size(); i++) {
				entity.resource.entity.util.EntityPair<String, Integer> breakpointLocationAndLine = lineBreakpoints.get(i);
				...
				if (breakpointLine.intValue() == currentLine && breakpointLocation.equals(location)) {
					// suspending...
					setSuspend(true);
					sendEvent(entity.resource.entity.debug.EEntityDebugMessageTypes.SUSPENDED, true, new String[] {"breakpoint", "" + currentLine});
					break;
				}
			}
			waitIfSuspended();
	...
	public void waitIfSuspended() {
		...
		sendEvent(entity.resource.entity.debug.EEntityDebugMessageTypes.RESUMED, true);
	}
	
	public void addLineBreakpoint(String location, int line) {
		lineBreakpoints.add(new entity.resource.entity.util.EntityPair<String, Integer>(location, new Integer(line)));
	}
	
	public void removeLineBreakpoint(String location, int line) {
		lineBreakpoints.remove(new entity.resource.entity.util.EntityPair<String, Integer>(location, new Integer(line)));
	}
	...
	public void terminate() {
		sendEvent(entity.resource.entity.debug.EEntityDebugMessageTypes.TERMINATED, false);
		stopEventSocket();
	}
	...
	public void startUpAndWait() {
		...
		if (isDebugMode()) {
			sendEvent(entity.resource.entity.debug.EEntityDebugMessageTypes.STARTED, true);
			setSuspend(true);
