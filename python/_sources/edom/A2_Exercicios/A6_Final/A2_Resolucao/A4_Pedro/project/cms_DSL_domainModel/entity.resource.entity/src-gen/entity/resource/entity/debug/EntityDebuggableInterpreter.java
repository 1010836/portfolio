 ...
 */
public class EntityDebuggableInterpreter<ResultType, ContextType> extends entity.resource.entity.debug.AbstractEntityDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private entity.resource.entity.util.AbstractEntityInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private entity.resource.entity.IEntityCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public EntityDebuggableInterpreter(entity.resource.entity.util.AbstractEntityInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new entity.resource.entity.IEntityInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = entity.resource.entity.util.EntityStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public entity.resource.entity.util.AbstractEntityInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		entity.resource.entity.IEntityLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private entity.resource.entity.IEntityLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		entity.resource.entity.IEntityCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(entity.resource.entity.debug.EEntityDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = entity.resource.entity.util.EntityEObjectUtil.getDepth(current);
		stopCondition = new entity.resource.entity.IEntityCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = entity.resource.entity.util.EntityEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = entity.resource.entity.util.EntityEObjectUtil.getDepth(current) - 1;
		stopCondition = new entity.resource.entity.IEntityCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
