...
public class EntityDebugTarget extends entity.resource.entity.debug.EntityDebugElement implements org.eclipse.debug.core.model.IDebugTarget, entity.resource.entity.debug.IEntityDebugEventListener {
	
	/**
	 * associated system process
	 */
	private entity.resource.entity.debug.EntityDebugProcess process;
	...
	private entity.resource.entity.debug.EntityDebugThread thread;
	 ...
	 * event listeners
	 */
	private java.util.List<entity.resource.entity.debug.IEntityDebugEventListener> eventListeners = new java.util.ArrayList<entity.resource.entity.debug.IEntityDebugEventListener>();
	
	private entity.resource.entity.debug.EntityDebugProxy debugProxy;
	...
	private class DebugEventDispatcher extends org.eclipse.core.runtime.jobs.Job {
		...
		public DebugEventDispatcher() {
			super(new entity.resource.entity.mopp.EntityMetaInformation().getSyntaxName() + " Event Dispatch");
			setSystem(true);
		}
		
		protected org.eclipse.core.runtime.IStatus run(org.eclipse.core.runtime.IProgressMonitor monitor) {
			while (!isTerminated()) {
				entity.resource.entity.debug.EntityDebugMessage message = communicationHelper.receive(eventReader);
				if (message != null) {
		...
		private void notifyListeners(entity.resource.entity.debug.EntityDebugMessage message) {
			Object[] listeners = eventListeners.toArray();
			for (int i = 0; i < listeners.length; i++) {
				((entity.resource.entity.debug.IEntityDebugEventListener) listeners[i]).handleMessage(message);
			}
		}
	}
	
	public EntityDebugTarget(entity.resource.entity.debug.EntityDebugProcess process, org.eclipse.debug.core.ILaunch launch, int requestPort, int eventPort) {
		super(launch.getDebugTarget());
		this.process = process;
		this.launch = launch;
		this.thread = new entity.resource.entity.debug.EntityDebugThread(this);
		this.threads = new org.eclipse.debug.core.model.IThread[] {this.thread};
		
		// initialize debug proxy
		try {
			this.debugProxy = new entity.resource.entity.debug.EntityDebugProxy(this, requestPort);
	 ...
	 */
	public void addEventListener(entity.resource.entity.debug.IEntityDebugEventListener listener) {
	 ...
	 * @param listener event listener
	 */
	public void removeEventListener(entity.resource.entity.debug.IEntityDebugEventListener listener) {
		eventListeners.remove(listener);
	}
	
	public String getName() throws org.eclipse.debug.core.DebugException {
		return new entity.resource.entity.mopp.EntityMetaInformation().getSyntaxName() + " model";
	}
		...
		if (supportsBreakpoint(breakpoint)) {
			try {
				if ((breakpoint.isEnabled() && getBreakpointManager().isEnabled()) || !breakpoint.isRegistered()) {
					entity.resource.entity.debug.EntityLineBreakpoint lineBreakpoint = (entity.resource.entity.debug.EntityLineBreakpoint) breakpoint;
	...
	public void handleMessage(entity.resource.entity.debug.EntityDebugMessage message) {
		try {
			if (message.hasType(entity.resource.entity.debug.EEntityDebugMessageTypes.STARTED)) {
				started();
			} else if (message.hasType(entity.resource.entity.debug.EEntityDebugMessageTypes.SUSPENDED)) {
				suspend();
			} else if (message.hasType(entity.resource.entity.debug.EEntityDebugMessageTypes.TERMINATED)) {
				terminated();
			} else if (message.hasType(entity.resource.entity.debug.EEntityDebugMessageTypes.RESUMED)) {
	...
	public entity.resource.entity.debug.EntityDebugProxy getDebugProxy() {
