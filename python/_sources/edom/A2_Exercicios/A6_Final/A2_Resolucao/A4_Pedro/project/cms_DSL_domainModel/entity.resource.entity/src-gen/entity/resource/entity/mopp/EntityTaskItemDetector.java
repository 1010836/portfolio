...
/**
 * The EntityTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * EntityTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class EntityTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<entity.resource.entity.mopp.EntityTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<entity.resource.entity.mopp.EntityTaskItem> foundItems = new java.util.ArrayList<entity.resource.entity.mopp.EntityTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new entity.resource.entity.mopp.EntityTaskItem(keyword, message, localLine, offset, end));
