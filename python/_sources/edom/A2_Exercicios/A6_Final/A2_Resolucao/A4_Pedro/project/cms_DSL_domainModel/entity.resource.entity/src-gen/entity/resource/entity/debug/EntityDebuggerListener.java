 ...
 * code).
 */
public class EntityDebuggerListener<ResultType, ContextType> implements Runnable {
	...
	private entity.resource.entity.debug.AbstractEntityDebuggable debuggable;
	/**
	 ...
	 */
	private java.util.Map<Long, entity.resource.entity.util.EntityPair<String, Object>> objectMap = new java.util.LinkedHashMap<Long, entity.resource.entity.util.EntityPair<String, Object>>();
	...
	public EntityDebuggerListener(int requestPort) {
		super();
	...
	private void runDebugger() throws java.io.IOException {
		...
		entity.resource.entity.debug.EntityDebugMessage command;
		while (!stop) {
			command = communicationHelper.receive(reader);
			if (command == null) {
				break;
			}
			if (command.hasType(entity.resource.entity.debug.EEntityDebugMessageTypes.EXIT)) {
				debuggable.terminate();
				stop = true;
			} else if (command.hasType(entity.resource.entity.debug.EEntityDebugMessageTypes.RESUME)) {
				debuggable.resume();
			} else if (command.hasType(entity.resource.entity.debug.EEntityDebugMessageTypes.STEP_OVER)) {
				debuggable.stepOver();
			} else if (command.hasType(entity.resource.entity.debug.EEntityDebugMessageTypes.STEP_INTO)) {
				debuggable.stepInto();
			} else if (command.hasType(entity.resource.entity.debug.EEntityDebugMessageTypes.STEP_RETURN)) {
				debuggable.stepReturn();
			} else if (command.hasType(entity.resource.entity.debug.EEntityDebugMessageTypes.ADD_LINE_BREAKPOINT)) {
			...
			} else if (command.hasType(entity.resource.entity.debug.EEntityDebugMessageTypes.REMOVE_LINE_BREAKPOINT)) {
				String location = command.getArgument(0);
				int line = Integer.parseInt(command.getArgument(1));
				debuggable.removeLineBreakpoint(location, line);
			} else if (command.hasType(entity.resource.entity.debug.EEntityDebugMessageTypes.GET_STACK)) {
				final String[] stack = debuggable.getStack();
				String controlStack = entity.resource.entity.util.EntityStringUtil.encode('#', stack);
				entity.resource.entity.debug.EntityDebugMessage message = new entity.resource.entity.debug.EntityDebugMessage(entity.resource.entity.debug.EEntityDebugMessageTypes.GET_STACK_RESPONSE, new String[] {controlStack});
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(entity.resource.entity.debug.EEntityDebugMessageTypes.GET_FRAME_VARIABLES)) {
				...
				entity.resource.entity.debug.EntityDebugMessage message = new entity.resource.entity.debug.EntityDebugMessage(entity.resource.entity.debug.EEntityDebugMessageTypes.GET_FRAME_VARIABLES_RESPONSE, topVariableIDs);
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(entity.resource.entity.debug.EEntityDebugMessageTypes.GET_VARIABLES)) {
				...
				entity.resource.entity.debug.EntityDebugMessage message = new entity.resource.entity.debug.EntityDebugMessage(entity.resource.entity.debug.EEntityDebugMessageTypes.GET_VARIABLES_RESPONSE, varStrings);
				communicationHelper.sendEvent(message, output);
			} else {
	...
	private String convertToString(long id, Object object) {
		...
		return entity.resource.entity.util.EntityStringUtil.convertToString(properties);
	}
	...
	private long getObjectID(String name, Object value) {
		entity.resource.entity.util.EntityPair<String, Object> pair = new entity.resource.entity.util.EntityPair<String, Object>(name, value);
	...
	public entity.resource.entity.debug.AbstractEntityDebuggable getDebuggable() {
		return debuggable;
	}
	
	public void setDebuggable(entity.resource.entity.debug.AbstractEntityDebuggable debuggable) {
