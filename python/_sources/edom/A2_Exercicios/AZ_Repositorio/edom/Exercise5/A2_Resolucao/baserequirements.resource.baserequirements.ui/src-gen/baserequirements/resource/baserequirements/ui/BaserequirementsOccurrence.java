 ...
 */
public class BaserequirementsOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "baserequirements.resource.baserequirements.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "baserequirements.resource.baserequirements.ui.occurences.declaration";
	...
	private baserequirements.resource.baserequirements.ui.BaserequirementsTokenScanner tokenScanner;
	 ...
	 */
	public BaserequirementsOccurrence(baserequirements.resource.baserequirements.IBaserequirementsTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, baserequirements.resource.baserequirements.ui.BaserequirementsTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new baserequirements.resource.baserequirements.mopp.BaserequirementsMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(baserequirements.resource.baserequirements.ui.BaserequirementsBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, baserequirements.resource.baserequirements.ui.BaserequirementsPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, baserequirements.resource.baserequirements.ui.BaserequirementsPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, baserequirements.resource.baserequirements.ui.BaserequirementsPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == baserequirements.resource.baserequirements.ui.BaserequirementsPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(baserequirements.resource.baserequirements.ui.BaserequirementsOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(baserequirements.resource.baserequirements.ui.BaserequirementsOccurrence.DECLARATION_ANNOTATION_ID);
	}
