 ...
 * contains
 * <code>baserequirements.resource.baserequirements.IBaserequirementsTextDiagnostic
 * </code>s it uses the more precise information of this extended diagnostic type.
 */
public class BaserequirementsMarkerHelper {
	
	/**
	 * The extension id of the custom marker type that is used by this text resource.
	 */
	public static final String MARKER_TYPE = baserequirements.resource.baserequirements.mopp.BaserequirementsPlugin.PLUGIN_ID + ".problem";
	...
	private static class MarkerCommandQueue {
		...
		private java.util.List<baserequirements.resource.baserequirements.IBaserequirementsCommand<Object>> commands = new java.util.ArrayList<baserequirements.resource.baserequirements.IBaserequirementsCommand<Object>>();
		
		private MutexRule schedulingRule = new MutexRule();
		
		public void addCommand(baserequirements.resource.baserequirements.IBaserequirementsCommand<Object> command) {
		...
		public void runCommands() {
			java.util.List<baserequirements.resource.baserequirements.IBaserequirementsCommand<Object>> commandsToProcess = new java.util.ArrayList<baserequirements.resource.baserequirements.IBaserequirementsCommand<Object>>();
			synchronized(commands) {
				commandsToProcess.addAll(commands);
				commands.clear();
			}
			for (baserequirements.resource.baserequirements.IBaserequirementsCommand<Object> command : commandsToProcess) {
	 ...
	 */
	public void mark(org.eclipse.emf.ecore.resource.Resource resource, baserequirements.resource.baserequirements.IBaserequirementsTextDiagnostic diagnostic) {
	...
	protected void createMarkerFromDiagnostic(final org.eclipse.core.resources.IFile file, final baserequirements.resource.baserequirements.IBaserequirementsTextDiagnostic diagnostic) {
		final baserequirements.resource.baserequirements.IBaserequirementsProblem problem = diagnostic.getProblem();
		baserequirements.resource.baserequirements.BaserequirementsEProblemType problemType = problem.getType();
		final String markerID = getMarkerID(problemType);
		COMMAND_QUEUE.addCommand(new baserequirements.resource.baserequirements.IBaserequirementsCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					if (problem.getSeverity() == baserequirements.resource.baserequirements.BaserequirementsEProblemSeverity.ERROR) {
					...
					baserequirements.resource.baserequirements.IBaserequirementsTextDiagnostic textDiagnostic = (baserequirements.resource.baserequirements.IBaserequirementsTextDiagnostic) diagnostic;
					...
					if (diagnostic instanceof baserequirements.resource.baserequirements.mopp.BaserequirementsResource.ElementBasedTextDiagnostic) {
						org.eclipse.emf.ecore.EObject element = ((baserequirements.resource.baserequirements.mopp.BaserequirementsResource.ElementBasedTextDiagnostic) diagnostic).getElement();
					...
					java.util.Collection<baserequirements.resource.baserequirements.IBaserequirementsQuickFix> quickFixes = textDiagnostic.getProblem().getQuickFixes();
					...
					if (!sourceIDs.isEmpty()) {
						marker.setAttribute(org.eclipse.core.resources.IMarker.SOURCE_ID, baserequirements.resource.baserequirements.util.BaserequirementsStringUtil.explode(sourceIDs, "|"));
					}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource) {
		for (baserequirements.resource.baserequirements.BaserequirementsEProblemType nextType : baserequirements.resource.baserequirements.BaserequirementsEProblemType.values()) {
	 ...
	 */
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, baserequirements.resource.baserequirements.BaserequirementsEProblemType problemType) {
		final org.eclipse.core.resources.IFile file = getFile(resource);
		if (file == null) {
			return;
		}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, final org.eclipse.emf.ecore.EObject causingObject) {
		...
		final String markerID = getMarkerID(baserequirements.resource.baserequirements.BaserequirementsEProblemType.UNKNOWN);
	 ...
	 * given type.
	 */
	public String getMarkerID(baserequirements.resource.baserequirements.BaserequirementsEProblemType problemType) {
	...
	protected void handleException(org.eclipse.core.runtime.CoreException ce) {
		if (ce.getMessage().matches("Marker.*not found.")) {
			...
			new baserequirements.resource.baserequirements.util.BaserequirementsRuntimeUtil().logError("Error while removing markers from resource:", ce);
		}
	}
		...
		COMMAND_QUEUE.addCommand(new baserequirements.resource.baserequirements.IBaserequirementsCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					baserequirements.resource.baserequirements.mopp.BaserequirementsPlugin.logError("Can't create marker.", e);
					return false;
				}
			}
		});
