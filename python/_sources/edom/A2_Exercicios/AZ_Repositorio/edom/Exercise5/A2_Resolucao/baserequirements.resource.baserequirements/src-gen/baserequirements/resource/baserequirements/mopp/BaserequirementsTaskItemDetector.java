...
/**
 * The BaserequirementsTaskItemDetector is used to find task items in text
 * documents. The current implementation searches for specific keywords to detect
 * task items. The BaserequirementsTaskItemDetector is used both by the
 * TaskItemBuilder and the editor.
 */
public class BaserequirementsTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<baserequirements.resource.baserequirements.mopp.BaserequirementsTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<baserequirements.resource.baserequirements.mopp.BaserequirementsTaskItem> foundItems = new java.util.ArrayList<baserequirements.resource.baserequirements.mopp.BaserequirementsTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new baserequirements.resource.baserequirements.mopp.BaserequirementsTaskItem(keyword, message, localLine, offset, end));
