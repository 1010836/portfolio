 ...
 */
public class BaserequirementsMinimalModelHelper {
	
	private final static baserequirements.resource.baserequirements.util.BaserequirementsEClassUtil eClassUtil = new baserequirements.resource.baserequirements.util.BaserequirementsEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + baserequirements.resource.baserequirements.util.BaserequirementsStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = baserequirements.resource.baserequirements.util.BaserequirementsListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + baserequirements.resource.baserequirements.util.BaserequirementsStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = baserequirements.resource.baserequirements.util.BaserequirementsListUtil.castListUnchecked(value);
