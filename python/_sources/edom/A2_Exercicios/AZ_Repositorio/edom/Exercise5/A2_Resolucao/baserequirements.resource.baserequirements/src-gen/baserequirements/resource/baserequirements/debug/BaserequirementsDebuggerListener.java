 ...
 * code).
 */
public class BaserequirementsDebuggerListener<ResultType, ContextType> implements Runnable {
	...
	private baserequirements.resource.baserequirements.debug.AbstractBaserequirementsDebuggable debuggable;
	/**
	 ...
	 */
	private java.util.Map<Long, baserequirements.resource.baserequirements.util.BaserequirementsPair<String, Object>> objectMap = new java.util.LinkedHashMap<Long, baserequirements.resource.baserequirements.util.BaserequirementsPair<String, Object>>();
	...
	public BaserequirementsDebuggerListener(int requestPort) {
		super();
	...
	private void runDebugger() throws java.io.IOException {
		...
		baserequirements.resource.baserequirements.debug.BaserequirementsDebugMessage command;
		while (!stop) {
			command = communicationHelper.receive(reader);
			if (command == null) {
				break;
			}
			if (command.hasType(baserequirements.resource.baserequirements.debug.EBaserequirementsDebugMessageTypes.EXIT)) {
				debuggable.terminate();
				stop = true;
			} else if (command.hasType(baserequirements.resource.baserequirements.debug.EBaserequirementsDebugMessageTypes.RESUME)) {
				debuggable.resume();
			} else if (command.hasType(baserequirements.resource.baserequirements.debug.EBaserequirementsDebugMessageTypes.STEP_OVER)) {
				debuggable.stepOver();
			} else if (command.hasType(baserequirements.resource.baserequirements.debug.EBaserequirementsDebugMessageTypes.STEP_INTO)) {
				debuggable.stepInto();
			} else if (command.hasType(baserequirements.resource.baserequirements.debug.EBaserequirementsDebugMessageTypes.STEP_RETURN)) {
				debuggable.stepReturn();
			} else if (command.hasType(baserequirements.resource.baserequirements.debug.EBaserequirementsDebugMessageTypes.ADD_LINE_BREAKPOINT)) {
			...
			} else if (command.hasType(baserequirements.resource.baserequirements.debug.EBaserequirementsDebugMessageTypes.REMOVE_LINE_BREAKPOINT)) {
				String location = command.getArgument(0);
				int line = Integer.parseInt(command.getArgument(1));
				debuggable.removeLineBreakpoint(location, line);
			} else if (command.hasType(baserequirements.resource.baserequirements.debug.EBaserequirementsDebugMessageTypes.GET_STACK)) {
				final String[] stack = debuggable.getStack();
				String controlStack = baserequirements.resource.baserequirements.util.BaserequirementsStringUtil.encode('#', stack);
				baserequirements.resource.baserequirements.debug.BaserequirementsDebugMessage message = new baserequirements.resource.baserequirements.debug.BaserequirementsDebugMessage(baserequirements.resource.baserequirements.debug.EBaserequirementsDebugMessageTypes.GET_STACK_RESPONSE, new String[] {controlStack});
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(baserequirements.resource.baserequirements.debug.EBaserequirementsDebugMessageTypes.GET_FRAME_VARIABLES)) {
				...
				baserequirements.resource.baserequirements.debug.BaserequirementsDebugMessage message = new baserequirements.resource.baserequirements.debug.BaserequirementsDebugMessage(baserequirements.resource.baserequirements.debug.EBaserequirementsDebugMessageTypes.GET_FRAME_VARIABLES_RESPONSE, topVariableIDs);
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(baserequirements.resource.baserequirements.debug.EBaserequirementsDebugMessageTypes.GET_VARIABLES)) {
				...
				baserequirements.resource.baserequirements.debug.BaserequirementsDebugMessage message = new baserequirements.resource.baserequirements.debug.BaserequirementsDebugMessage(baserequirements.resource.baserequirements.debug.EBaserequirementsDebugMessageTypes.GET_VARIABLES_RESPONSE, varStrings);
				communicationHelper.sendEvent(message, output);
			} else {
	...
	private String convertToString(long id, Object object) {
		...
		return baserequirements.resource.baserequirements.util.BaserequirementsStringUtil.convertToString(properties);
	}
	...
	private long getObjectID(String name, Object value) {
		baserequirements.resource.baserequirements.util.BaserequirementsPair<String, Object> pair = new baserequirements.resource.baserequirements.util.BaserequirementsPair<String, Object>(name, value);
	...
	public baserequirements.resource.baserequirements.debug.AbstractBaserequirementsDebuggable getDebuggable() {
		return debuggable;
	}
	
	public void setDebuggable(baserequirements.resource.baserequirements.debug.AbstractBaserequirementsDebuggable debuggable) {
