 ...
 */
public class BaserequirementsBracketSet {
	...
	private java.util.ArrayList<baserequirements.resource.baserequirements.IBaserequirementsBracketPair> bracketPairs;
	 ...
	 */
	private class BracketPair implements baserequirements.resource.baserequirements.IBaserequirementsBracketPair {
	 ...
	 */
	private class ClosingListener implements baserequirements.resource.baserequirements.ui.IBaserequirementsBracketHandler, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.ModifyListener, org.eclipse.swt.custom.VerifyKeyListener {
		...
		public void verifyText(org.eclipse.swt.events.VerifyEvent e) {
			...
			if (caret > 0 && caret < textWidget.getCharCount()) {
				baserequirements.resource.baserequirements.IBaserequirementsBracketPair bracketPair = getBracketPair(textWidget.getTextRange(caret - 1, 1), textWidget.getTextRange(caret, 1));
	 ...
	 */
	public BaserequirementsBracketSet(baserequirements.resource.baserequirements.ui.BaserequirementsEditor editor, org.eclipse.jface.text.source.ISourceViewer sourceViewer) {
		languageID = new baserequirements.resource.baserequirements.mopp.BaserequirementsMetaInformation().getSyntaxName();
		this.bracketPairs = new java.util.ArrayList<baserequirements.resource.baserequirements.IBaserequirementsBracketPair>();
	 ...
	 */
	public baserequirements.resource.baserequirements.IBaserequirementsBracketPair getBracketPair(String opening, String closing) {
		for (baserequirements.resource.baserequirements.IBaserequirementsBracketPair bracketPair : bracketPairs) {
	...
	public baserequirements.resource.baserequirements.IBaserequirementsBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public boolean setClosingEnabledInside(baserequirements.resource.baserequirements.IBaserequirementsBracketPair bracketPair, boolean closingEnabledInside) {
	...
	public boolean resetBrackets() {
		String bracketPairs = preferenceStore.getString(languageID + baserequirements.resource.baserequirements.ui.BaserequirementsPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public baserequirements.resource.baserequirements.IBaserequirementsBracketPair remove(String opening, String closing) {
		for (baserequirements.resource.baserequirements.IBaserequirementsBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public boolean setBrackets(String bracketSet) {
		...
		bracketPairs = new java.util.ArrayList<baserequirements.resource.baserequirements.IBaserequirementsBracketPair>();
	...
	public void matchingBrackets() {
		...
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, baserequirements.resource.baserequirements.ui.BaserequirementsPositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, baserequirements.resource.baserequirements.ui.BaserequirementsPositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
