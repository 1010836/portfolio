 ...
 */
public class BaserequirementsDebuggableInterpreter<ResultType, ContextType> extends baserequirements.resource.baserequirements.debug.AbstractBaserequirementsDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private baserequirements.resource.baserequirements.util.AbstractBaserequirementsInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private baserequirements.resource.baserequirements.IBaserequirementsCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public BaserequirementsDebuggableInterpreter(baserequirements.resource.baserequirements.util.AbstractBaserequirementsInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new baserequirements.resource.baserequirements.IBaserequirementsInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = baserequirements.resource.baserequirements.util.BaserequirementsStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public baserequirements.resource.baserequirements.util.AbstractBaserequirementsInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		baserequirements.resource.baserequirements.IBaserequirementsLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private baserequirements.resource.baserequirements.IBaserequirementsLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		baserequirements.resource.baserequirements.IBaserequirementsCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(baserequirements.resource.baserequirements.debug.EBaserequirementsDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = baserequirements.resource.baserequirements.util.BaserequirementsEObjectUtil.getDepth(current);
		stopCondition = new baserequirements.resource.baserequirements.IBaserequirementsCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = baserequirements.resource.baserequirements.util.BaserequirementsEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = baserequirements.resource.baserequirements.util.BaserequirementsEObjectUtil.getDepth(current) - 1;
		stopCondition = new baserequirements.resource.baserequirements.IBaserequirementsCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
