...
public class BaserequirementsDefaultNameProvider implements baserequirements.resource.baserequirements.IBaserequirementsNameProvider {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = baserequirements.resource.baserequirements.util.BaserequirementsEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
