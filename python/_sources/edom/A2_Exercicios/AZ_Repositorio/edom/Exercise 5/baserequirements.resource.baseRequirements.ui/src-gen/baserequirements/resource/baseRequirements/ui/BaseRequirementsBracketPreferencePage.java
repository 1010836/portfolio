 ...
 * </ul>
 */
public class BaseRequirementsBracketPreferencePage extends org.eclipse.jface.preference.PreferencePage implements org.eclipse.ui.IWorkbenchPreferencePage {
	
	private static final String[] ALL_LEFT_BRACKETS = new String[] { "{", "(", "[", "<", "\"", "'", };
	private static final String[] ALL_RIGHT_BRACKETS = new String[] { "}", ")", "]", ">", "\"", "'", };
	
	private String BRACKETS_COLOR = baserequirements.resource.baseRequirements.ui.BaseRequirementsPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR;
	...
	private String language = new baserequirements.resource.baseRequirements.mopp.BaseRequirementsMetaInformation().getSyntaxName();
	
	private baserequirements.resource.baseRequirements.ui.BaseRequirementsBracketSet bracketsTmp;
	
	/**
	 * Creates a preference page for bracket setting.
	 */
	public BaseRequirementsBracketPreferencePage() {
		super();
		
		baserequirements.resource.baseRequirements.IBaseRequirementsMetaInformation metaInformation = new baserequirements.resource.baseRequirements.mopp.BaseRequirementsMetaInformation();
	...
	public void init(org.eclipse.ui.IWorkbench workbench) {
		setPreferenceStore(baserequirements.resource.baseRequirements.ui.BaseRequirementsUIPlugin.getDefault().getPreferenceStore());
		setDescription("Define the coloring of matching brackets.");
		
		bracketsTmp = new baserequirements.resource.baseRequirements.ui.BaseRequirementsBracketSet(null, null);
		for (String languageID : languageIDs) {
			bracketSetTemp.put(languageID, getPreferenceStore().getString(languageID + baserequirements.resource.baseRequirements.ui.BaseRequirementsPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		}
	}
	...
	private void handleMatchingBracketsSelection() {
		...
		enableCheckbox.setSelection(getPreferenceStore().getBoolean(		baserequirements.resource.baseRequirements.ui.BaseRequirementsPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		enableClosingInside.setSelection(false);
		matchingBracketsColorButton.setEnabled(getPreferenceStore().getBoolean(		baserequirements.resource.baseRequirements.ui.BaseRequirementsPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		...
		initializeLanguage();
		bracketsTmp.setBrackets(getPreferenceStore().getString(language + baserequirements.resource.baseRequirements.ui.BaseRequirementsPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		...
		bracketsList.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int index : itemIndices) {
					baserequirements.resource.baseRequirements.IBaseRequirementsBracketPair bracketPair = bracketsTmp.getBracketPair(index);
		...
		enableClosingInside.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int idx : itemIndices) {
					baserequirements.resource.baseRequirements.IBaseRequirementsBracketPair bracketPair = bracketsTmp.getBracketPair(idx);
	...
	protected void performDefaults() {
		enableCheckbox.setSelection(getPreferenceStore().getDefaultBoolean(baserequirements.resource.baseRequirements.ui.BaseRequirementsPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		matchingBracketsColorButton.setEnabled(enableCheckbox.getSelection());
		...
		bracketSetTemp.put(language, getPreferenceStore().getDefaultString(language + baserequirements.resource.baseRequirements.ui.BaseRequirementsPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		bracketsTmp.setBrackets(bracketSetTemp.get(language));
		bracketsList.setItems(bracketsTmp.getBracketArray());
		enableClosingInside.setSelection(false);
	}
	...
	private void updateActiveEditor() {
		...
		getPreferenceStore().setValue(baserequirements.resource.baseRequirements.ui.BaseRequirementsPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX, enableCheckbox.getSelection());
		getPreferenceStore().setValue(language + baserequirements.resource.baseRequirements.ui.BaseRequirementsPreferenceConstants.EDITOR_BRACKETS_SUFFIX, bracketSetTemp.get(language));
		...
		if (editor != null && editor instanceof baserequirements.resource.baseRequirements.ui.BaseRequirementsEditor) {
			((baserequirements.resource.baseRequirements.ui.BaseRequirementsEditor) editor).invalidateTextRepresentation();
		}
	}
