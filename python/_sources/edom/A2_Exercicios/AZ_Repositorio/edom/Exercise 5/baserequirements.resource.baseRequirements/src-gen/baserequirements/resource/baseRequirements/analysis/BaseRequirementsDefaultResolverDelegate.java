...
public class BaseRequirementsDefaultResolverDelegate<ContainerType extends org.eclipse.emf.ecore.EObject, ReferenceType extends org.eclipse.emf.ecore.EObject> {
	...
	private static baserequirements.resource.baseRequirements.mopp.BaseRequirementsMetaInformation metaInformation = new baserequirements.resource.baseRequirements.mopp.BaseRequirementsMetaInformation();
	
	private baserequirements.resource.baseRequirements.IBaseRequirementsNameProvider nameProvider = metaInformation.createNameProvider();
	 ...
	 */
	protected void resolve(String identifier, ContainerType container, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, baserequirements.resource.baseRequirements.IBaseRequirementsReferenceResolveResult<ReferenceType> result) {
		try {
	 ...
	 */
	protected boolean tryToResolveIdentifierInObjectTree(String identifier, org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EObject root, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, baserequirements.resource.baseRequirements.IBaseRequirementsReferenceResolveResult<ReferenceType> result, boolean checkRootFirst) {
	...
	protected boolean tryToResolveIdentifierAsURI(String identifier, ContainerType container, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, baserequirements.resource.baseRequirements.IBaseRequirementsReferenceResolveResult<ReferenceType> result) {
		org.eclipse.emf.ecore.EClass type = reference.getEReferenceType();
	...
	protected boolean tryToResolveIdentifierInGenModelRegistry(String identifier, ContainerType container, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, baserequirements.resource.baseRequirements.IBaseRequirementsReferenceResolveResult<ReferenceType> result) {
		org.eclipse.emf.ecore.EClass type = reference.getEReferenceType();
	...
	protected boolean checkElement(org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EObject element, org.eclipse.emf.ecore.EReference reference, int position, org.eclipse.emf.ecore.EClass type, String identifier, boolean resolveFuzzy, boolean checkStringWise, baserequirements.resource.baseRequirements.IBaseRequirementsReferenceResolveResult<ReferenceType> result) {
		...
		if (exactMatch == null) {
			...
			if (similarMatch != null) {
				...
				result.addQuickFix(new baserequirements.resource.baseRequirements.mopp.BaseRequirementsChangeReferenceQuickFix("Replace with " + similarMatch, "IMG_TOOL_FORWARD", container, reference, oldTarget, element));
			}
			return true;
		}
	...
	protected String produceDeResolveErrorMessage(org.eclipse.emf.ecore.EObject refObject, org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EReference reference, baserequirements.resource.baseRequirements.IBaseRequirementsTextResource resource) {
	...
	protected baserequirements.resource.baseRequirements.IBaseRequirementsReferenceCache getCache(org.eclipse.emf.ecore.EObject object) {
		org.eclipse.emf.ecore.EObject root = org.eclipse.emf.ecore.util.EcoreUtil.getRootContainer(object);
		org.eclipse.emf.common.notify.Adapter adapter = baserequirements.resource.baseRequirements.util.BaseRequirementsEObjectUtil.getEAdapter(root, baserequirements.resource.baseRequirements.analysis.BaseRequirementsReferenceCache.class);
		baserequirements.resource.baseRequirements.analysis.BaseRequirementsReferenceCache cache = baserequirements.resource.baseRequirements.util.BaseRequirementsCastUtil.cast(adapter);
		if (cache != null) {
			return cache;
		} else {
			// cache does not exist. create a new one.
			cache = new baserequirements.resource.baseRequirements.analysis.BaseRequirementsReferenceCache(nameProvider);
	...
	protected boolean isSimilar(String identifier, Object attributeValue) {
		if (attributeValue != null && attributeValue instanceof String) {
			String name = (String) attributeValue;
			if (baserequirements.resource.baseRequirements.util.BaseRequirementsStringUtil.computeLevenshteinDistance(identifier, name) <= MAX_DISTANCE) {
				return true;
			}
		}
		return false;
